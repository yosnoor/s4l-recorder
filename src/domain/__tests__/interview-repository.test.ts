...
describe("AsyncStorageInterviewRepository", () => {
  ...
  describe("findById", () => {
    it("retrieves stored interview", async () => {
      ...
      expect(AsyncStorage.getItem).toHaveBeenCalledWith("s4l_interview_test-id");
    });
  ...
  describe("findAll", () => {
    it("returns all interviews sorted by updatedAt descending", async () => {
      const olderInterview = {
        ...mockInterview,
        metadata: { ...mockInterview.metadata, id: "old", updatedAt: "2026-08-22T10:00:00Z" }
      };
      const newerInterview = {
        ...mockInterview,
        metadata: { ...mockInterview.metadata, id: "new", updatedAt: "2026-08-23T10:00:00Z" }
      };
      ...
      (AsyncStorage.multiGet as jest.Mock).mockResolvedValue([
        ["s4l_interview_old", JSON.stringify({ version: 1, data: olderInterview })],
        ["s4l_interview_new", JSON.stringify({ version: 1, data: newerInterview })]
      ]);
  ...
  describe("update", () => {
    it("updates existing interview", async () => {
      ...
      const updated = {
        ...mockInterview,
        metadata: { ...mockInterview.metadata, intervieweeName: "Updated Name" }
      };
  ...
  describe("delete", () => {
    it("removes interview by ID", async () => {
      ...
      expect(AsyncStorage.removeItem).toHaveBeenCalledWith("s4l_interview_test-id");
    });
  });
  ...
  describe("Corrupt/empty data handling", () => {
    ...
    it("findAll skips corrupt items", async () => {
        (AsyncStorage.getAllKeys as jest.Mock).mockResolvedValue(["s4l_interview_bad", "s4l_interview_good"]);
        (AsyncStorage.multiGet as jest.Mock).mockResolvedValue([
            ["s4l_interview_bad", "invalid json {"],
            ["s4l_interview_good", JSON.stringify({ version: 1, data: mockInterview })]
        ]);
